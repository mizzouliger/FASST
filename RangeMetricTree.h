//
// Created by Seth Wiesman on 11/29/15.
//

#ifndef THESIS_RANGEMETRICTREE_H
#define THESIS_RANGEMETRICTREE_H
...
>
class RangeMetricTree {
public:
    RangeMetricTree(std::vector<T> points) {
...
private:
    ...
    std::shared_ptr<Node> build_tree(std::vector<std::shared_ptr<Node>> points, std::size_t low, std::size_t high) {
        ...
        points[low]->points_in_radius.reserve(mid - low + 1);
        for (auto i = low + 1; i < mid; i++) {
            points[low]->points_in_radius.push_back(points[i]->point);
        }
    ...
    void search(std::shared_ptr<Node> root, std::vector<T>& result, const T& target, double radius) {
        if (root == nullptr) {
            return;
        }
        ...
        if (dist + root->innerRadius <= radius) {
            result.insert(result.end(), root->points_in_radius.begin(), root->points_in_radius.end());
        } else if (dist - radius <= root->innerRadius) {
...
#endif //THESIS_RANGEMETRICTREE_H
